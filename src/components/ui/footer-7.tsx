...
export const Footer7 = ({
  ...
  return <section className="py-16 bg-[#003250] text-white">
      <div className="container mx-auto px-4">
        ...
        once: true
          ...
          <motion.div className="flex w-full flex-col justify-between gap-6 lg:items-start" variants={itemVariants}>
            ...
            <ul className="flex items-center space-x-6 text-gray-300">
              {socialLinks.map((social, idx) => <li key={idx} className="font-medium hover:text-cyan-400 transition-colors">
          ...
          <motion.div className="grid w-full gap-6 md:grid-cols-3 lg:gap-20" variants={itemVariants}>
            {sections.map((section, sectionIdx) => <div key={sectionIdx}>
                ...
                <ul className="space-y-3 text-sm text-gray-300">
                  {section.links.map((link, linkIdx) => <li key={linkIdx} className="font-medium hover:text-cyan-400 transition-colors">
                      {link.href.startsWith('http') || link.href.startsWith('mailto:') || link.href.startsWith('https://wa.me') ? <a href={link.href} target="_blank" rel="noopener noreferrer">{link.name}</a> : <Link to={link.href}>{link.name}</Link>}
                    </li>)}
                </ul>
              </div>)}
          </motion.div>
        ...
        <motion.div className="mt-8 flex flex-col justify-between gap-4 border-t border-gray-600 py-8 text-xs font-medium text-gray-300 md:flex-row md:items-center md:text-left" variants={itemVariants} initial="hidden" whileInView="visible" viewport={{
        once: true
      }}>
          <p className="order-2 lg:order-1">{copyright}</p>
          <ul className="order-1 flex flex-col gap-2 md:order-2 md:flex-row">
            {legalLinks.map((link, idx) => <li key={idx} className="hover:text-cyan-400 transition-colors">
